...
class DialogManager(object):
    ...
    def train(self, user: SimulatedUser, max_num_episodes=1000, max_num_actions=30, gamma=0.99,
              ...
              store_policy=True, policy_file='policy.csv', episode_history_file='rewards.csv'):
...
if __name__ == '__main__':
    user = SimulatedUser(irrelevant_prob_provide=0.2, irrelevant_prob_confirm=0.1, neg_confirm_prob=0.1)
    DM = DialogManager()
    DM.train(user, max_num_episodes=5000, exploration_rate=1, explore_decay_rate=0.001, store_policy=True)
